...
//	}
	...
	DistanceMatrix::constructDistanceMatrix(distance_matrix, room_map, chosen_positions, 1.0, 0.0, map_resolution, path_planner_);
	...
	for(size_t pose=start-best_order.begin(); fow_path.size()!=chosen_poses.size(); ++pose)
	{
		...
		// insert pose
		fow_path.push_back(chosen_poses[best_order[pose]]);
	}

	// if the plan should be retrieved for the footprint return it here
	...
	// ************* V. Map the fow path to the robot path if wanted. *************
	ROS_INFO("Starting to map from field of view pose to robot pose");
	mapPath(room_map, path, fow_path, robot_to_fow_middlepoint_vector, map_resolution, map_origin, starting_position);

//	testing
//	cv::Mat test_map = room_map.clone();
//	for(size_t i=0; i<cell_centers.size(); ++i)
//		cv::circle(test_map, cell_centers[i], 4, cv::Scalar(127), CV_FILLED);
//	for(size_t i=0; i<path.size(); ++i)
//		cv::circle(test_map, cv::Point(path[i].x/map_resolution, path[i].y/map_resolution), 2, cv::Scalar(100), CV_FILLED);
