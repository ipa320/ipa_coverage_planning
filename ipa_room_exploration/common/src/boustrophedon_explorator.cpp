...
//		This allows to use the algorithm as it was and in the last step, simply rotating the found path points back to the original
//		orientation.
...
//		}
		...
		double dist1 = path_planner_.planPath(rotated_room_map, robot_pos, path_lines[0].left_edge_, 1.0, 0.0, map_resolution);
		double dist2 = path_planner_.planPath(rotated_room_map, robot_pos, path_lines[0].right_edge_, 1.0, 0.0, map_resolution);
		double dist3= path_planner_.planPath(rotated_room_map, robot_pos, path_lines.back().left_edge_, 1.0, 0.0, map_resolution);
		double dist4 = path_planner_.planPath(rotated_room_map, robot_pos, path_lines.back().right_edge_, 1.0, 0.0, map_resolution);
		...
		if(start_from_upper_path == true) // plan the path starting from upper horizontal line
		{
			for(std::vector<boustrophedonHorizontalLine>::iterator line=path_lines.begin(); line!=path_lines.end(); ++line)
			{
				...
				if(left == true) // plan path to left and then to right edge
				{
					...
					path_planner_.planPath(rotated_room_map, robot_pos, line->left_edge_, 1.0, 0.0, map_resolution, 0, &astar_path);
				...
				else // plan path to right then to left edge
				{
					...
					std::vector<cv::Point> astar_path;
					path_planner_.planPath(rotated_room_map, robot_pos, line->right_edge_, 1.0, 0.0, map_resolution, 0, &astar_path);
					for(size_t path_point=0; path_point<astar_path.size(); ++path_point)
					{
						if(cv::norm(robot_pos - astar_path[path_point]) >= path_eps)
						{
