#include <ipa_room_segmentation/voronoi_segmentation_server.h>

voronoi_segmentation_algorithm::voronoi_segmentation_algorithm(std::string name_of_the_action) :
		voronoi_segmentation_server_(nh_, name_of_the_action, boost::bind(&voronoi_segmentation_algorithm::execute_vor_segmentation_server, this, _1),
		        false), action_name_(name_of_the_action)
{
	//Start action server
	voronoi_segmentation_server_.start();
...
void voronoi_segmentation_algorithm::execute_vor_segmentation_server(const ipa_room_segmentation::MapSegmentationGoalConstPtr &goal)
{
	ros::Rate looping_rate(1);
	ROS_INFO("*****Segmentation action server with voronoi Method********");
	...
	//publish result
	voronoi_segmentation_server_.setSucceeded(action_result_);
	...
	cv::imwrite("/home/rmb-fj/Pictures/maps/action_tests/voronoi.png", segmented_map_);
}

int main(int argc, char** argv)
{
	ros::init(argc, argv, "voronoi_segmentation_server");
	voronoi_segmentation_algorithm segmentationAlgorithmObj(ros::this_node::getName());
	ROS_INFO("Action Server for voronoi segmentation has been initalized......");
	ros::spin();
