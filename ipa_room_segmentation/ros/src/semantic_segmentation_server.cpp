#include <ipa_room_segmentation/semantic_segmentation_server.h>

semantic_segmentation_algorithm::semantic_segmentation_algorithm(std::string name_of_the_action, bool train) :
		semantic_segmentation_server_(nh_, name_of_the_action, boost::bind(&semantic_segmentation_algorithm::execute_sem_segmentation_server, this, _1), false), action_name_(
		        name_of_the_action)
{
	train_the_algorithm_ = train;
	//check if algorithm needs to be trained. train_the_algorithm_ is set in semantic_segmentation_server.h
	if (train_the_algorithm_)
	{
		//load the training maps
		cv::Mat first_room_training_map = cv::imread("src/autopnp/ipa_room_segmentation/training_maps/room_training_map.png", 0);
		cv::Mat second_room_training_map = cv::imread("src/autopnp/ipa_room_segmentation/training_maps/maps/lab_d_room_training_map.png", 0);
		cv::Mat first_hallway_training_map = cv::imread("src/autopnp/ipa_room_segmentation/training_maps/hallway_training_map.png", 0);
		cv::Mat second_hallway_training_map = cv::imread("src/autopnp/ipa_room_segmentation/training_maps/lab_a_hallway_training_map.png", 0);
		//train the algorithm
		segmenter_.trainClassifiers(first_room_training_map, second_room_training_map, first_hallway_training_map, second_hallway_training_map);
	}
	//Start action server
	semantic_segmentation_server_.start();
...
void semantic_segmentation_algorithm::execute_sem_segmentation_server(const ipa_room_segmentation::MapSegmentationGoalConstPtr &goal)
{
	ros::Rate looping_rate(1);
	ROS_INFO("*****Segmentation action server with semantic labeling Method********");
	...
	segmented_map_ = segmenter_.semanticLabeling(original_img);
	...
	//publish result
	semantic_segmentation_server_.setSucceeded(action_result_);
	...
	cv::imwrite("/home/rmb-fj/Pictures/maps/action_tests/semantic_test.png", segmented_map_);
}

int main(int argc, char** argv)
{
	ros::init(argc, argv, "semantic_segmentation_server");
	semantic_segmentation_algorithm segmentationAlgorithmObj(ros::this_node::getName(), true);
	ROS_INFO("Action Server for semantic segmentation has been initalized......");
	ros::spin();
